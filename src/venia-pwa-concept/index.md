...
The `module` directory contains the code for a helper module for the Venia theme.
...
* Assigns a `RootComponent` to an entity or group of entities without requiring a deployment
* Embeds GraphQL payloads into the initial server render without requiring a round trip to the API after the web app has initialized on the client side
